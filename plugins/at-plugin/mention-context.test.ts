...
describe("untrusted text helpers", () => {
  it("strips C0/C1 controls and normalizes whitespace", () => {
    expect(normalizeUntrustedText(" \u0000one\t\n two\u007f\u0085 three  ")).toBe(
      "one two three",
    );
  });
  ...
  it("rejects blank and overlong stable identities", () => {
    ...
    expect(normalizeStableIdentity("x".repeat(MAX_IDENTITY_BYTES + 1))).toBeNull();
  });
});
...
describe("agent-visible plugin contexts", () => {
  it("constructs the exact approved Installed template", () => {
    expect(buildInstalledPluginContext({ name: "GitHub", pluginId: "github" })).toBe(
      [
  ...
  it("caps overlong Community metadata without cutting fixed instructions", () => {
    ...
    expect(context).toContain("The mention itself is not installation consent.");
  ...
  it("does not leak descriptions, capabilities, settings, or diagnostics", () => {
    const context = buildInstalledPluginContext({ name: "GitHub", pluginId: "github" });

    expect(context).not.toMatch(/description|capability list|settings|secret|path|diagnostic/i);
  });
});
